...
void move_to_memory(CPU *cpu, uint8_t *src){
  ...
  uint16_t combined_registers =
      combine_registers(cpu->registers.H, cpu->registers.L);
  cpu->memory[combined_registers] = *src;
  return;
}
...
void move_immediate_to_memory(CPU *cpu, uint8_t src,
                              uint16_t dest) { // MVI M, data
  cpu->memory[dest] = src;
  return;
}

void load_reg_pair_immed(uint8_t reg_high, uint8_t reg_low,
                         uint16_t *register_pair) { // LXI
                                                    // rp,
                                                    // data16
...
void store_acc_direct(CPU *cpu, uint8_t high_byte,
                      uint8_t low_byte) { // STA addr
...
void store_h_l_direct(CPU *cpu, uint8_t high_byte,
                      uint8_t low_byte) { // SHLD addr
