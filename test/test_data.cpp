...
TEST(load_reg_pair_immed, test_3) {
  ...
  uint16_t BC = combine_registers(cpu.registers.B, cpu.registers.A);
  load_reg_pair_immed(byte_three, byte_two, &BC);
  ASSERT_EQ(BC, 0b1111100000001111);
}
...
TEST(store_acc_direct, test_1) {
  CPU cpu = {0};
  uint8_t byte_three = 0xAF; // 0xFF
  uint8_t byte_two = 0xBB;   // 0x00
  cpu.memory[0xAFBB] = 255;
  cpu.registers.A = 20;
  store_acc_direct(&cpu, byte_three, byte_two);
  ASSERT_EQ(cpu.memory[0xAFBB], 20);
}
